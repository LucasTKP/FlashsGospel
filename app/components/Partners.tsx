        ...
        <div className="container mx-auto flex flex-col items-center mt-14 px-4">
            ...
            <div className="flex mt-10 gap-10 overflow-x-scroll">
                {partnersImages[0].image_Patrocinadores.map((image: any) => {
                    return <Image key={image.ref} src={urlFor(image).url()} alt="Patrocinadores" width={300} height={300} className="border-2 border-yellow-500 rounded-sm max-sm:w-[200px] max-sm:h-[150px]"></Image>
                })}
            </div>
        </div>
    )
